...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      <header className="flex items-center justify-between mb-8 border-b border-neutral-200 pb-4">
        <h1 className="text-lg font-semibold">勤務時間</h1>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        ...
        <button
          ...
          className="border border-neutral-700 bg-neutral-800 text-white rounded-md px-4 py-2 text-sm font-medium hover:border-neutral-600"
        >
          Thêm mới
        </button>
      </div>
      ...
      {loading ? (
        <ul className="space-y-2" aria-hidden>
          {[1, 2, 3, 4].map((i) => (
            <li
              key={i}
              className="flex items-center justify-between border border-neutral-200 rounded-md px-4 py-3 animate-pulse"
            >
              <span className="h-5 w-24 rounded bg-neutral-200" />
              <span className="h-5 w-20 rounded bg-neutral-100" />
              <div className="flex gap-2">
                <span className="h-8 w-12 rounded bg-neutral-100" />
                <span className="h-8 w-12 rounded bg-neutral-100" />
              </div>
            </li>
          ))}
        ...
        <section className="border border-neutral-200 rounded-lg p-8 text-center">
          <h2 className="text-base font-semibold text-neutral-800 mb-2">Chưa có bản ghi nào</h2>
          <p className="text-sm text-neutral-600 mb-6">Năm {year} chưa có tháng nào. Thêm tháng đầu tiên để bắt đầu ghi 勤務時間.</p>
          <button
            type="button"
            onClick={() => setModalOpen(true)}
        ...
        {missingMonths.length > 0 && (
          <section className="mb-6 border border-neutral-200 rounded-lg p-4">
            <h2 className="text-sm font-medium text-neutral-700 mb-2">Tháng chưa có trong {year}年</h2>
              ...
              {missingMonths.map((m) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-md px-3 py-1.5 text-sm border focus:outline-none focus:ring-2 focus:ring-neutral-400 focus:ring-inset ${
                      tooFar
        ...
        <ul className="space-y-2">
          {months.map((m) => (
            <li
              key={`${m.year}-${m.month}`}
              className="flex items-center justify-between border border-neutral-200 rounded-md px-4 py-3"
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="border border-neutral-300 rounded-md px-3 py-1 text-sm hover:border-neutral-500"
                >
                  Xem
                </button>
                <button
                  ...
                  className="border border-neutral-700 rounded-md px-3 py-1 text-sm hover:bg-neutral-100"
                >
                  Sửa
                </button>
              </div>
      ...
      {modalOpen && (
        ...
        >
          <div
            ref={modalContentRef}
            className="bg-white border border-neutral-200 rounded-lg p-6 w-full max-w-sm"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 id="modal-title" className="text-lg font-semibold mb-4">Thêm tháng mới</h2>
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-neutral-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-400 focus:ring-inset"
                ...
                {(isMonthExists(modalMonth) || isMonthTooFar(modalMonth)) && (
                  <p id="month-hint" className="text-xs text-neutral-500 mt-1">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">開始 (giờ bắt đầu)</label>
                <input
                  type="time"
              ...
              <div>
                <label className="block text-sm font-medium mb-1">終了 (giờ kết thúc)</label>
                <input
                  type="time"
                  value={modalTimeEnd}
              ...
              <div>
                <label className="block text-sm font-medium mb-1">休憩 (phút)</label>
                <input
                  type="number"
                  min={0}
                  value={modalBreak}
            ...
            <div className="flex gap-2 mt-6">
              <button
                type="button"
                onClick={() => setModalOpen(false)}
                className="flex-1 border border-neutral-300 rounded-md py-2 text-sm"
              >
                Hủy
              </button>
              <button
                ...
                className="flex-1 border border-neutral-700 bg-neutral-800 text-white rounded-md py-2 text-sm font-medium"
              >
                Xác nhận
              </button>
            </div>
      ...
      {logoutConfirmOpen && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 id="logout-modal-title" className="text-lg font-semibold text-neutral-800 mb-2">
              Xác nhận đăng xuất
            </h2>
            <p className="text-sm text-neutral-600 mb-6">Bạn có chắc muốn đăng xuất?</p>
