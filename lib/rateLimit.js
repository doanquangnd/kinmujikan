 ...
 */
export async function check_rate_limit(req, endpoint) {
  ...
  try {
    ...
    const current_count = expired ? 0 : parseInt(row.request_count, 10) || 0;
    const new_count = current_count + 1;

    if (new_count > limit) {
      const retryAfter = row
        ? Math.ceil((new Date(row.window_expires_at) - now) / 1000)
        : WINDOW_SECONDS;
